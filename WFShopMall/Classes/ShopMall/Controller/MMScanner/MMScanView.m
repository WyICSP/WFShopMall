...
@property (nonatomic, assign) NSInteger heightScale;//扫描框的大小 1 代表的是二维码扫描框
...
-(id)initWithFrame:(CGRect)frame style:(NSString *)style
{
    if (self = [super initWithFrame:frame])
    {
        ...
        self.heightScale = 1;
        NSString *path = [NSString getImagePathWithCurrentBundler:[NSBundle bundleForClass:[self class]] PhotoName:@"scan_blue_line" bundlerName:@"WFShopMall.bundle"];
        self.lineImageView = [[UIImageView alloc] initWithImage:[UIImage imageWithContentsOfFile:path]];
...
- (void)layoutSubviews {
    ...
    _lineImageView.frame = CGRectMake(Left + 20, YMinRetangle + 2, sizeRetangle.width - 4, 5);
...
- (void)startAnimating {
    ...
    CGRect initFrame = CGRectMake(Left+20, YMinRetangle + 2, sizeRetangle.width - 4, 5);
...
- (void)drawScanRect {
    ...
    CGContextSetStrokeColorWithColor(context, [UIColor colorWithRed:255/255.0 green:110/255.0 blue:37/255.0 alpha:1.00].CGColor);

...
typedef NS_ENUM(NSInteger, WFJumpScanType) {
    WFJumpScanByApp = 0,//原生跳转
    WFJumpScanByH5 //H5 跳转
};

@interface MMScanViewController : YFBaseViewController

@property (nonatomic, strong) UILabel *tipTitle;  //扫码区域下方提示文字

@property (nonatomic, strong) UIView *toolsView;  //底部显示的功能项 -box
/// 扫描或者输入返回信息
@property (nonatomic, copy) void(^scanInfoBlock)(NSDictionary *codeInfo);
/**
 导航条按钮
 */
...
/**
 跳转方式, 来判断返回结果
 */
@property (nonatomic, assign) WFJumpScanType jumpScanType;
